...
class Printful_Shipping extends WC_Shipping_Method
{
    ...
    public function calculate_shipping($package = array())
    {
        ...
        foreach ($package['contents'] as $item) {
            ...
            $customized_variant_id = $item['variation_id'] ? $item['variation_id'] : $item['product_id'];
            $customized_variant_author = $this->getAuthor($customized_variant_id);
            if($customized_variant_author == '999'){
                //TG Shirt
                $customized_variant_id = 9022;
            }elseif($customized_variant_author == '998'){
                //TG Tote
                $customized_variant_id = 9022;
            }
            $request['items'] []= array(
                'external_variant_id' => $customized_variant_id,
